...
class ProduitService {
    ...
    async getProductsByBoutique(boutiqueId, queryParams = {}) {
        ...
        // ✅ on enlève min/max du filtre DB pour filtrer ensuite sur prixFinal
        const filter = this.buildFilters({ ...queryParams, minPrice: undefined, maxPrice: undefined });
        filter.boutique = boutiqueId;

        const sort = this.buildSortOptions(queryParams.sortBy, queryParams.order);

        // ✅ on récupère tout (pour que filtre + pagination soient cohérents)
        const [allData, promoActive] = await Promise.all([
            Produit.find(filter).sort(sort).lean(),
            this.getActivePromotionForBoutique(boutiqueId),
        ]);

        const dataWithPromo = (allData || []).map((p) => {
            const applies = this.promoAppliesToProduct(promoActive, p._id);
            const computed = this.computePrixFinal(p.prix, applies ? promoActive : null);
        ...
        const max =
            queryParams.maxPrice !== undefined &&
                ...
                String(queryParams.maxPrice) !== ""
                ? Number(queryParams.maxPrice)
                : null;

        let filtered = dataWithPromo;
        if (min != null) filtered = filtered.filter((p) => Number(p.prixFinal ?? p.prix) >= min);
        if (max != null) filtered = filtered.filter((p) => Number(p.prixFinal ?? p.prix) <= max);
        ...
        return {
            success: true,
            data: paged,
            pagination: this.getPaginationMeta(filtered.length, page, limit),
            filters: {
                boutique: boutiqueId,
                search: queryParams.search || "",
                minPrice: queryParams.minPrice || "",
    ...
    // =========================
    // PROMOTIONS (prix calculé)
    // =========================

    async getActivePromotionForBoutique(boutiqueId) {
        const now = new Date();

        // ✅ règle simple : 1 promo active max, sinon on prend la plus forte
        const promo = await Promotion.findOne({
            boutique: boutiqueId,
            dateDebut: { $lte: now },
            dateFin: { $gte: now },
        })
            .sort({ pourcentage: -1, created_at: -1, _id: -1 })
            .lean();

        return promo || null;
    }

    computePrixFinal(prix, promo) {
        const prixOriginal = Number(prix ?? 0);

        if (!promo) return { prixOriginal, prixFinal: prixOriginal, promotion: null };

        const remise = Number(promo.pourcentage ?? 0);
        const prixFinal = Math.max(0, Math.round(prixOriginal * (1 - remise / 100)));
        ...
        return {
            ...
            promotion: {
                ...
                scope: promo.scope,
                event: promo.event ?? null,
            },
        };
    }

    promoAppliesToProduct(promo, produitId) {
        ...
        if (promo.scope === "PRODUCTS") {
            ...
            return ids.some((x) => String(x) === String(produitId));
        }
    ...
    async getProduitById(id) {
        ...
        const promoActive = await this.getActivePromotionForBoutique(produit.boutique);

        const applies = this.promoAppliesToProduct(promoActive, produit._id);
        const computed = this.computePrixFinal(produit.prix, applies ? promoActive : null);
